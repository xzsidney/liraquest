...
4. **⚔️ Arena de Batalha 2D (RPG de Turnos com Sprites Clássicos MUGEN & Efeitos Visuais RPG Maker via PixiJS):**
   - **Mecânica Tática por Turnos (Estilo Final Fantasy & Summoners War):** Combate 2D estratégico com barra visual de Fila de Turnos (CTB Queue no topo), pontos de vida (HP), pontos de magia (MP) e barra de Fúria (0 a 100%).
   - **Motor de Efeitos Visuais PixiJS (WebGL Additive FX — Estilo RPG Maker):**
     - Sobreposto ao canvas 2D de sprites, um motor dedicado de partículas e iluminação WebGL em modo aditivo (`PIXI.BLEND_MODES.ADD`) renderiza feixes de luz radiantes (*Radiant Sunburst Rays* com 12 a 24 pontas luminosas rotativas que expandem e dissipam suavemente), anéis concêntricos de ondas de choque (*Shockwave Rings*), faíscas incandescentes em corte de espada/garra e cartas cinéticas flutuantes.
     - **Cinematics de Hyper Arcane:** O acionamento de Golpes Supremos dispara o *Hyper Flash*, tremor de câmera proporcional ao impacto e explosão estelar com feixes duplos contrarrotativos em cores personalizadas por herói.
   - **Fila Dinâmica de Ordem de Turnos (CTB Turn Queue):**
     - Barra nobre no topo da arena exibindo os próximos 4 turnos de combate em formato de pílulas iluminadas, destacando com brilho pulsante dourado o combatente ativo e permitindo antecipar ações táticas de defesa e cura.
   - **Conexão Direta com a Ficha de Atributos do Herói:**
     ...
     - **Ataque Mágico / Projétil de Energia:** Baseado em Inteligência (`INT`) (`35 + INT * 3.5`).
   ...
   - **Deck Modular de Ações (1 Ataque Básico + 3 Habilidades Equipadas):**
     ...
     - **Desacoplamento Visual via `animation_id`:** As habilidades no banco de dados contêm um ponteiro numérico (`animation_id`), permitindo que novas habilidades criadas pelo administrador utilizem animações da engine (1 = arremesso/projétil, 2 = feixe contínuo, 3 = gancho ascendente, 4 = investida/dash, 5 = rasteira, 100 = básico, 600 = golpe supremo) sem reescrever código de batalha.
   - **Elenco Completo de 5 Campeões Lendários:**
     - **🛡️ Capitão América (Tanque / Vanguarda):** Combate com o lendário Escudo de Vibranium. Possui ataques físicos diretos (*Soco Direto*), arremesso perfurante à distância (*Shield Slash* com projétil dinâmico que vai e volta), gancho aéreo ascendente (*Stars & Stripes* com atordoamento), investida de impacto (*Charging Star*) e o Golpe Supremo *FINAL JUSTICE* com explosão cósmica dourada.
     - **🔴 Ciclope (Artilheiro / Mestre Óptico):** Mestre de controle de campo à distância. Utiliza chutes acrobáticos (*Cyclone Kick*), rajada óptica contínua (*Optic Blast* com feixe laser contínuo), varredura de plasma (*Optic Sweep*), gancho ascendente (*Gene Splice*) e o Golpe Supremo *MEGA OPTIC BLAST*.
     - **🕷️ Homem-Aranha (Ágil / Mestre das Teias):** Focado em Agilidade e Sorte. Golpes acrobáticos rápidos (*Combo Aranha*), disparo de projétil de teia com rede de contenção (*Web Ball* que pode atordoar o adversário), voadora com balanço em teia (*Web Swing*), gancho duplo perfurante (*Spider Sting*) e o Golpe Supremo *MAXIMUM SPIDER* com rajadas cruzadas de luz radiante branca e azul celeste.
     - **🐺 Wolverine (Fúria / Crítico Devastador):** Focado em Força e Sorte com altíssimo dano crítico. Cortes velozes (*Corte de Adamantium*), tempestade de cortes furiosos com garras incandescentes (*Berserker Barrage*), gancho ascendente cortante (*Tornado Claw*), mergulho em parafuso perfurante (*Drill Claw*) e o Golpe Supremo *BERSERKER RAGE* com tempestade de cortes dourados e rubros.
     - **🃏 Gambit (Mestre Cinético / Arcano Tático):** Focado em Inteligência e Agilidade. Golpes ágeis com bastão telescópico (*Golpe de Bastão Bo*), disparo de cartas cinéticas carregadas com explosão de energia violeta (*Kinetic Card*), salto com impacto descendente (*Cajun Slash*), investida rápida com bastão energizado (*Cajun Strike*) e o Golpe Supremo *ROYAL FLUSH* com detonação de leque de cartas e ondas de choque magenta e ciano.
...
5. **🎞️ Visualizador & Editor de Animações 2D (`/animator.html`):**
   - Ferramenta visual do Arcade para inspeção, calibração e criação de golpes dos 5 lutadores (Capitão América, Ciclope, Homem-Aranha, Wolverine e Gambit).
