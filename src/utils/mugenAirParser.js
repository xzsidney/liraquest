export function parseMugenAir(airContent, existingFiles) {
    ...
    for (const rawLine of lines) {
        ...
        if (currentActionId !== null) {
            ...
            if (line.startsWith('Clsn') || line.startsWith('Loopstart'))
                continue;
    ...
    const getActionFrames = (id) => allActions[id] || [];
    ...
    if (special.length === 0)
        special = getActionFrames(100);
    if (special.length === 0)
        special = getActionFrames(10000);
    if (special.length === 0)
        special = attackHeavy;
    // Hit: Ação 5000 (head/face hit) ou 5010 (body hit) ou 5001/5002
    let hit = getActionFrames(5000);
    if (hit.length === 0)
        hit = getActionFrames(5010);
    if (hit.length === 0)
        hit = getActionFrames(5001);
    if (hit.length === 0)
        hit = idle.slice(0, 2);
    // Win: Ação 180 ou 181
    let win = getActionFrames(180);
    if (win.length === 0)
        win = getActionFrames(181);
    return {
